...
const menuSections: MenuSection[] = [
  {
    ...
    items: [
      ...
      { icon: ClipboardList, label: "Prontuarios", path: "/prontuarios" },
  ...
  },
  {
    label: "Atendimento",
    items: [
      { icon: FileText, label: "Prescricoes", path: "/prescricoes" },
      { icon: Stethoscope, label: "Exames", path: "/exames" },
      { icon: ShieldCheck, label: "Assinaturas", path: "/assinaturas" },
      { icon: Receipt, label: "Orcamentos", path: "/orcamentos" },
      { icon: Wallet, label: "Fiscal", path: "/fiscal", adminOnly: true },
    ],
  },
  {
    label: "Operacional",
    items: [
      ...
      { icon: MessageSquareText, label: "CRM", path: "/crm" },
      ...
      { icon: ShieldCheck, label: "Usuarios", path: "/usuarios", adminOnly: true },
      ...
      <div className="relative flex min-h-screen items-center justify-center overflow-hidden px-6 py-12">
        ...
        <div className="relative flex w-full max-w-xl flex-col items-center gap-8 rounded-[2rem] border border-gold/25 bg-surface/75 px-10 py-12 text-center shadow-[0_40px_100px_rgba(90,63,18,0.16)] backdrop-blur-2xl dark:bg-surface/70">
          <div className="app-brand-mark flex h-24 w-24 items-center justify-center p-4">
            <img src="/logo-glutee.png" alt="Glutec" className="h-full w-full object-contain" />
          </div>
          <div className="space-y-3">
            <p className="text-xs font-semibold uppercase tracking-[0.4em] text-text-tertiary">
              Clinica Wesley Camara
            </p>
            <h1 className="text-4xl font-light tracking-tight text-text-primary">
              Plataforma premium para atendimento, gestao e prontuario.
            </h1>
            <p className="mx-auto max-w-md text-sm leading-7 text-text-secondary">
              Entre para continuar a migracao dos legados, revisar a emissao fiscal e operar a clinica com a nova identidade visual.
            </p>
          </div>
          <Button
            onClick={() => {
  ...
  const activeMenuItem = useMemo(
    () =>
      sections
        ...
        .find(item => matchesPath(location, item.path)),
    [location, sections]
  );
            ...
            <div className="flex items-center gap-3">
              ...
              {!isCollapsed ? (
                <div className="min-w-0">
                  <p className="truncate text-lg font-semibold tracking-[0.18em] text-text-primary">
                    CLUTEC
                  </p>
                  <p className="truncate text-[11px] uppercase tracking-[0.28em] text-text-tertiary">
                    Clinica Ciate
                  </p>
                </div>
              ) : null}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="app-sidebar-user flex w-full items-center gap-3 rounded-2xl px-2 py-2 text-left transition-colors hover:bg-accent/10 group-data-[collapsible=icon]:justify-center focus:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  <Avatar className="h-10 w-10 border border-gold/20 shrink-0">
      ...
      <SidebarInset className="app-main-shell">
        <header className="app-main-header hidden h-20 items-center justify-between px-6 md:flex">
          <div className="flex min-w-0 items-center gap-4">
            <div className="rounded-full border border-gold/25 bg-white/45 px-3 py-1 text-xs font-semibold uppercase tracking-[0.28em] text-text-tertiary dark:bg-white/5">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-lg font-semibold text-text-primary">
                Ambiente de atendimento e gestao centralizado
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Button variant="outline" className="rounded-xl" onClick={() => setLocation("/pacientes")}>
              <Users className="h-4 w-4" />
              Novo paciente
            </Button>
            <Button variant="premium" className="rounded-xl" onClick={() => setLocation("/agenda")}>
        ...
        <main className="flex-1 px-4 pb-6 pt-4 lg:px-6 lg:pb-8 lg:pt-6">{children}</main>
      </SidebarInset>
    </>
  );
}
